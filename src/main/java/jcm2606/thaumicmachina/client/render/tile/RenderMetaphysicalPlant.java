...
public class RenderMetaphysicalPlant
extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileMetaphysicalRose tile, double x, double y, double z) {
        ...
        if (player.getCurrentArmor(slot) != null && (stack = player.getCurrentArmor(slot)).getItem() != null && (item = stack.getItem()) instanceof IRevealer) {
            ...
            tess.setBrightness(tile.getWorldObj().getBlock(tile.xCoord, tile.yCoord, tile.zCoord).getMixedBrightnessForBlock((IBlockAccess)tile.getWorldObj(), tile.xCoord, tile.yCoord, tile.zCoord));
            Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation("ThaumicMachina:textures/blocks/metaphysicalRose.png"));
            this.drawCrossedSquares(tile.getWorldObj().getBlock(tile.xCoord, tile.yCoord, tile.zCoord).getIcon(0, 0), x, y, z, 1.0f, tess);
            tess.draw();
            GL11.glPopMatrix();
        }
    }

    public void drawCrossedSquares(IIcon icon, double par3, double par5, double par7, float par9, Tessellator tessellator) {
        double d3 = 0.0;
        double d4 = 0.0;
        double d5 = 1.0;
        double d6 = 1.0;
