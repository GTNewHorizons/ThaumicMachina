...
public class TileNodeTransmodifier
extends TMTileEntity
implements IInventory {
                ...
                for (int j = 0 - (regionBounds / 2 + 3); j < regionBounds; ++j) {
                    for (int k = 0 - (regionBounds / 2 + 3); k < regionBounds; ++k) {
                        ...
                        for (int l = 0; l < this.slots.length; ++l) {
                            ...
                            Item item;
                            ItemStack stack;
                            if (this.slots[l] == null || (stack = this.slots[l]).getItem() == null || !((item = stack.getItem()) instanceof ItemNodeAugmentation)) continue;
                            ...
                            if (ItemNodeAugmentation.getAugmentation(stack) == null || !(augmentation = ItemNodeAugmentation.getAugmentation(stack)).compatible(augmentationList)) continue;
                            ItemNodeAugmentation.getAugmentation(stack).handle(node);
                            b = true;
                        }
    ...
    public void drawBeam(TileNode node) {
        ...
        FXBeam fx = new FXBeam(this.getWorldObj(), (double)this.xCoord + 0.5, (double)this.yCoord + 0.75, (double)this.zCoord + 0.5, (double)node.xCoord + 0.5, (double)node.yCoord + 0.5, (double)node.zCoord + 0.5, 10, 1, "ThaumicMachina:textures/misc/beam_transmodifier.png");
        fx.speed = 0.015;
        fx.colour = c.getRGB();
        fx.opacity = 0.25f;
        fx.noClip = false;
    ...
    public void doubleRecharge(TileNode node) {
        try {
            Method method = node.getClass().getDeclaredMethod("handleRecharge", Boolean.TYPE);
