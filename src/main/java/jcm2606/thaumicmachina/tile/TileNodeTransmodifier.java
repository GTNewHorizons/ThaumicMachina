...
import jcm2606.thaumicmachina.core.helper.ColorHelper;
...
public class TileNodeTransmodifier extends TMTileEntity implements IInventory {
    ...
    public final ItemStack[] slots = new ItemStack[4];
    ...
    public void updateEntity() {
        ...
        final int regionBounds = 7;
        for (int i = -(regionBounds / 2 + 3); i < regionBounds; ++i) {
            for (int j = -(regionBounds / 2 + 3); j < regionBounds; ++j) {
                for (int k = -(regionBounds / 2 + 3); k < regionBounds; ++k) {
                    TileEntity tile = this.worldObj.getTileEntity(this.xCoord + i, this.yCoord + j, this.zCoord + k);
                    if (!(tile instanceof TileNode node)) continue;
                    for (ItemStack stack : this.slots) {
                        if (stack == null || (stack).getItem() == null
                            || !(stack.getItem() instanceof ItemNodeAugmentation)) continue;
    ...
    public void drawBeam(TileNode node) {
        ...
        float progress = (float) Math.sqrt(Math.pow(MathHelper.sin((float) this.getTicks() / 20.0f), 2.0));
        Color c = ColorHelper.blendColors(fractions, colours, progress);
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        System.arraycopy(loadedSlots, 0, this.slots, 0, loadedSlots.length);
    }
