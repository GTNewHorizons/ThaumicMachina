...
public class WandAugmentationHandler {
    public static int handleMaxVisMethod(ItemStack stack) {
        ...
        if (stack != null && stack.getItem() != null && stack.getItem() instanceof ItemWandCasting) {
            ...
            maxVis = wand.getRod(stack).getCapacity() * (wand.isSceptre(stack) ? 150 : 100);
    ...
    public static float handleConsumptionModifierMethod(ItemStack stack, boolean specialCost) {
        ...
        if (stack != null && stack.getItem() != null && stack.getItem() instanceof ItemWandCasting) {
            ItemWandCasting wand = (ItemWandCasting)stack.getItem();
            f = !specialCost ? wand.getCap(stack).getBaseCostModifier() : wand.getCap(stack).getSpecialCostModifier();
    ...
    public static double handleRuneRendererDouble(ItemStack stack) {
        ...
        double d = 0.36;
        if (stack != null && stack.getItem() != null && stack.getItem() instanceof ItemWandCasting && !((wand = (ItemWandCasting)stack.getItem()).getRod(stack) instanceof StaffRod)) {
